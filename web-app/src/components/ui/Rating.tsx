...
export const Rating: FC<RatingProps> = ({
  ...
  return (
    <div
      className={['ui-rating inline-flex items-center gap-1.5', className].filter(Boolean).join(' ')}
      role={readOnly ? 'img' : 'radiogroup'}
      aria-label={ariaLabel || `Оцінка: ${value} з ${maxStars}`}
    >
      <div className="ui-rating__stars inline-flex gap-1">
        {Array.from({ length: maxStars }).map((_, index) => {
          ...
          if (readOnly) {
            return (
              <svg
                ...
                fill="none"
                aria-hidden="true"
                className="ui-rating__star shrink-0"
              >
                <path
                  d={starFullD}
          ...
          return (
            <button
              ...
              aria-label={`Оцінити ${starIndex} зірок`}
              onMouseEnter={() => setHoverValue(starIndex)}
              onMouseLeave={() => setHoverValue(null)}
              onClick={() => onChange?.(starIndex)}
              className="bg-transparent border-0 p-0 cursor-pointer transition-transform hover:scale-110 outline-none flex items-center justify-center"
            >
              <svg
                width={size}
                height={size}
      ...
      {showScore && <span className="ui-rating__score text-sm font-accented font-semibold text-content-dark">{value.toFixed(1)}</span>}
    </div>
  );
};
