...
export const Rating: FC<RatingProps> = ({
  ...
  return (
    <div className={['ui-rating', className].filter(Boolean).join(' ')} role="img" aria-label={`Оцінка: ${value} з ${maxStars}`}>
        ...
        {Array.from({ length: maxStars }).map((_, index) => {
          ...
          return (
            <svg
              ...
              aria-label={!readOnly ? `Оцінити ${starIndex} зірок` : undefined}
              className={`ui-rating__star ${!readOnly ? 'ui-rating__star--interactive cursor-pointer' : ''}`}
