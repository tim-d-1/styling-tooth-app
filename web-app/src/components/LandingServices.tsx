    ...
    <section id="services" className="w-full py-20 bg-landing-page">
      <div className="max-w-[1200px] mx-auto px-6 flex flex-col gap-10">
        <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between gap-6">
          <div className="max-w-[692px] flex flex-col gap-5">
            <h2 className="font-accented font-bold text-3xl md:text-[40px] leading-tight text-content-dark">
              Наші послуги
            </h2>
            <p className="font-accented text-base leading-relaxed text-content-dark max-w-[590px]">
          ...
          <button
            ...
            className="w-full lg:w-[488px] h-12 rounded-xl bg-terracotta hover:bg-terracotta-hover text-surface-cream font-accented font-semibold text-base flex items-center justify-center transition-colors shadow-sm cursor-pointer shrink-0"
          >
            Швидкий запис
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {LANDING_SERVICES.map((service) => (
            <div
              key={service.id}
              className="bg-surface-cream rounded-[10px] shadow-card px-6 pt-7 pb-9 flex flex-col items-center text-center justify-between transition-transform hover:-translate-y-0.5"
            >
